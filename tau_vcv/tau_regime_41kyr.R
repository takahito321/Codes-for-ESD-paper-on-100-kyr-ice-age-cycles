...
tau0<-95
postscript(file="tau_regime_41kyr.eps",onefile=FALSE,horizontal=FALSE,encoding="WinAnsi.enc",width=8,height=7)
par(mfrow=c(1,1))
par(mar=c(0,0,0,0))
par(oma=c(1,0,0,0))
...
lines(px[regime==4,1],px[regime==4,2],type="p",pch=4,col="violet",cex=cx,lwd=2)
...
legend("bottomright",c("82 kyr","41 kyr"),col=c("violet","dodgerblue"),cex=cx,pch=c(4,5),bg="white")
mtext(expression(paste("Timescale control parameter r")), side = 1, line = 2.5, col=1, cex=cx)
title("VCV18 model with 41-kyr periodic forcing",line=0.6,cex=cx)
...
mtext(expression(paste("Internal timescale ",r,"T"[0]," (kyr)")),1,line=7,at=0.75,col=1,cex=cx)
abline(h=0)
dev.off()
