...
for(i in 1:nrow(px)){
    ...
    amplitude[pj[i,1],pj[i,2]]<-sd(xt[10000:20001])
    #if(amplitude[pj[i,1],pj[i,2]]>5){
...
amplitude2[amplitude>80]<-80

tau0<-95
postscript(file="tau_vcv.eps",onefile=FALSE,horizontal=FALSE,encoding="WinAnsi.enc",width=8,height=7)
par(mfrow=c(1,1))
par(mar=c(0,0,0,0))
par(oma=c(1,0,0,0))
...
cl<-viridis(4)
cx<-1.4
image.plot(tau,w,amplitude2,col=brewer.pal(9,"Purples")[1:9],ylim=c(min(w),2),zlim=c(0,80),ylab="Forcing amplitude A",xlab="",cex.lab=cx,cex.axis=cx,cex=cx)
...
legend("bottomright",c("124 kyr","95 kyr","82 kyr","41 kyr","23.7 kyr"),col=c(cl[1],2,"pink","dodgerblue","darkorange"),cex=cx,pch=c(2,3,4,5,6),bg="white")
...
#postscript(file="tau_vcv_no_points.eps",onefile=FALSE,horizontal=FALSE,encoding="WinAnsi.enc",width=8,height=7)
par(mfrow=c(1,1))
par(mar=c(0,0,0,0))
par(oma=c(1,0,0,0))
par(mai = c(1.5, 1, 0.5, 0.6))
...
postscript(file="tau_vcv_amplitude.eps",onefile=FALSE,horizontal=FALSE,encoding="WinAnsi.enc",width=8,height=7)
par(mfrow=c(1,1))
par(mar=c(0,0,0,0))
par(oma=c(1,0,0,0))
par(mai = c(1.5, 1, 0.5, 0.6))
