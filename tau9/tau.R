...
for(i in 1:nrow(px)){
    ...
    amplitude[pj[i,1],pj[i,2]]<-sd(xt[ix>mean(ix)])
    if(amplitude[pj[i,1],pj[i,2]]>0.1){
...
amplitude2[amplitude>0.8]<-0.8

tau0<-91.7
postscript(file="tau.eps",onefile=FALSE,horizontal=FALSE,encoding="WinAnsi.enc",width=8,height=7)
par(mfrow=c(1,1))
par(mar=c(0,0,0,0))
par(oma=c(1,0,0,0))
...
cl<-viridis(4)
cx<-1.4
image.plot(tau,w,amplitude2,col=brewer.pal(9,"Purples")[1:9],ylim=c(min(w),2),zlim=c(0,0.8),ylab="Forcing amplitude A",xlab="",cex.lab=cx,cex.axis=cx,cex=cx)
...
legend("bottomright",c("405 kyr","124 kyr","95 kyr","82 kyr","41 kyr","23.7 kyr","22.4 kyr","18.9 kyr"),col=c(cl[3],cl[1],2,"pink","dodgerblue","darkorange",7,8),cex=cx,pch=c(1,2,3,4,5,6,7,8),bg="white")
